...
jsPsych.plugins['image-slider-2AFC'] = (function() {

  var plugin = {};

  jsPsych.pluginAPI.registerPreload('image-slider-2AFC', 'stimulus', 'image');

  plugin.info = {
    name: 'image-slider-2AFC',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
   ...
   //  }
    ...
    html += '<input type="range" class="slider" value="'+trial.start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" id="jspsych-image-slider-response-response" onkeydown="event.preventDefault()"></input>';
     // add submit button
