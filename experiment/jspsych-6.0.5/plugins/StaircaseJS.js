function Staircase(stairs) {
  ...
  for (var i in stairs) {
    ...
    this.stairs[i].withReset = stairs[i].withReset || false; // if true, the values of consecutive trials get reset after change
        ...
        wait: {
          '1': function(stair) {
            ...
            if (stair.successiveBad>=stair.up) {
              ...
              if (stair.withReset) stair.successiveBad = 0;
        ...
        wait: {
          '1': function(stair) {
            ...
            if (stair.successiveGood>=stair.down) {
              if (stair.withReset) stair.successiveGood = 0;
              return stair.val[stair.val.length-1]+stair.stepSizeUp;
            } else {
              return stair.val[stair.val.length-1];
            }
