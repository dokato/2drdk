/*
	...
	MODIFIED by Dominik Krzeminski (2020).
...
jsPsych.plugins["RDK"] = (function() {
	...
	plugin.info = {
	    ...
	    parameters: {
		    ...
		    colour_config: {
		      type: jsPsych.plugins.parameterType.INT,
		      pretty_name: "Colour config",
		      default: 1,
		      description: "The configuration of colors: 1 normal, 2 inverted"
		    },
		    rotation_config: {
				type: jsPsych.plugins.parameterType.INT,
				pretty_name: "Rotation config",
				default: 1,
				description: "Rotation config with angle (degrees)"
			  },  
			border_color: {
		      type: jsPsych.plugins.parameterType.STRING,
	...
	plugin.trial = function(display_element, trial) {
		...
		trial.colour_config = assignParameterValue(trial.colour_config, 1);
		trial.rotation_config = assignParameterValue(trial.rotation_config, 0);
		...
		*/
		...
		var colourConfig = trial.colour_config; //colour of the rectangles
		var borderColor = trial.border_color; //The color of the border
		var rotationConfig = trial.rotation_config; //rotation
		...
		function end_trial() {
			...
			var trial_data = { 
				...
				"border": trial.border,
				"colour_config": trial.colour_config,
				"rotation_config": trial.rotation_config,
		...
		function setUpMultipleApertures(){
			...
			colourConfigArray = setParameter(colourConfig);
		...
		function initializeCurrentApertureParameters(){
			...
			colourConfig = colourConfigArray[currentApertureNumber];
      ...
      //Three for loops that do things in sequence: clear, update, and draw dots.
	    ...
	    function clearDots(){
			...
			rot = rotationConfig[currentApertureNumber];
			ctx.save();
			ctx.translate(apertureCenterX, apertureCenterY);
		...
		function draw() {
			...
			if (Array.isArray(rotationConfig) === false)
			...
			ctx.translate(-apertureCenterX, -apertureCenterY);
				...
				var shift = 50;
				var strech = 100;
				var col1 = colourConfig == 1 ? "red" : "blue";
				var col2 = colourConfig == 1 ? "blue" : "red";
				ctx.fillStyle   = col1;  // making rectangles
				ctx.fillRect(apertureCenterX - shift, apertureCenterY + verticalAxis, strech, shift);
				ctx.fillStyle   = "white";
				canvas_arrow(ctx, apertureCenterX, apertureCenterY, apertureCenterX, apertureCenterY+verticalAxis+30, 10);
