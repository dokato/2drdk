...
jsPsych.plugins['image-slider-response'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var html ='';
    html += '<img src="' + trial.symbol + '">' ;
    if (trial.prompt !== null){
      html += '<font size="100%">' + trial.prompt + '</font>';
    };
    html += '<div id="jspsych-image-slider-response-wrapper" style="margin: 40px 0px;  height: 100%; width = 100%">';
    html += '<div id="jspsych-image-slider-response-stimulus" ><img style="width:100%; height: 100%" src="' + trial.stimulus + '"></div>';
    html += '<div class="jspsych-image-slider-response-container" style="position:relative;">';
    html += '<input type="range" value="'+trial.start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" onkeydown="event.preventDefault()" class = "slider" id="jspsych-image-slider-response-response"></input>';
    html += '<div>'
    for(var j=0; j < trial.labels.length; j++){
      ...
      //html+=trial.labels[j]
      html += '<span style="text-align: center; font-size: 125%;">'+trial.labels[j]+'</span>';
      html += '</div>' //<span
    }
    html += '</div>';
    html += '</div>';
