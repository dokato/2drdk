...
def euler_fast(func, x0, t, threshold):
    """
    Faster version of Euler integration that cares about values
    of state variables and interrupts when the threshold was reached.
    """
    dt = t[1] - t[0]
    x = np.zeros((len(t), len(x0)))
    x[0] = x0
...
def calculate_input_B_cong(coherence, mu = mu_o):
    "Calculate input for axis B in congruent case"
    I_mot_l = J_ext * mu * (1 + coherence *1. / 100)
    I_mot_r = J_ext * mu * (1 - coherence *1. / 100)
    return I_mot_l, I_mot_r

def calculate_input_B_incon(coherence, mu = mu_o):
    "Calculate input for axis B in incongruent case"
    I_mot_l = J_ext * mu * (1 - coherence *1. / 100)
    I_mot_r = J_ext * mu * (1 + coherence *1. / 100)
    return I_mot_l, I_mot_r
...
def model(x, t):
    '''
    This function runs one time-step of W&W model based on
    differentail equations from (Wong&Wang, 2006).
    '''
    global I_o, cA, cB
    sl, sr, I_n1, I_n2 = x
...
def run_for_params(mu_ratio, i0_ratio, silent = True):
    ...
    I_o = I_o_1
    calculate_input_B = lambda x: calculate_input_B_incon(x, 0)
    calculate_input_A = lambda x: calculate_input_A_mu(x, mu_o)
    cA, cB = 15, 0
    ...
    I_o = I_o_1
    calculate_input_B = lambda x: calculate_input_B_cong(x, 0)
    calculate_input_A = lambda x: calculate_input_A_mu(x, mu_o)
    cA, cB = 20, 0
    if not silent: print('sim 1d')
...
if __name__ == "__main__":
    ...
    Nsample = 50
    mu_range = np.linspace(0.5, 10, Nsample)
